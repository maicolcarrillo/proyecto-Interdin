    ...
    <div>
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-gray-800 mb-2">
                Configuración BCT Bank - Datos Cargados
            </h1>
...
//  Mapeo de cuotas a código
const getCreditCode = (installment) => {
    if (installment === 9) return 'CA1';
    if (installment === 12) return 'CAD';
    return `CUOTAS_${installment}`;
};

// Condiciona la description a visualizar
const getCreditDescription = (code) => {
    if (code === 'CAD') return 'PLAN DE CUOTAS COMERCIOS';
    return 'PAGOS A CUOTAS'
...
const jsonStructure = computed(() => {
    ...
    cleanedData.value.forEach(row => {
        const typesValue = row.typesCredit;
        if (!typesValue) return;

        // Extrae TODOS los números del texto (ej: "9 Y 12")
        const numbers = typesValue.match(/\d+/g);
        if (!numbers) return;

        numbers.forEach(num => {
            const installment = parseInt(num);
            const code = getCreditCode(installment);


            if (!creditsMap.has(code)) {
                creditsMap.set(code, {
                    code,
                    description: getCreditDescription(code),
                    installment,
    ...
    return {
        include: [{
            ranges: uniqueRanges,
            credits: Array.from(creditsMap.values())
        }]
    };
});
...
const displayedJson = computed(() =>
    showCompressed.value ? compressedJson.value : formattedJson.value
);
